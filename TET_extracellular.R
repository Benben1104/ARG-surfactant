...
bartlett.test(Extracellular ~ Group, data=d_SDS)
...
ggsave("TET+SDS_ex.png", width=690/90, height=420/90, dpi=600, unit="in")

  

d_DTAC <- read.csv("TET+DTAC.csv")
head(d_DTAC)
...
shapiro.test(residuals)
bartlett.test(Extracellular ~ Group, data=d_DTAC)
...
ggsave("TET+DTAC_ex.png", width=690/90, height=420/90, dpi=600, unit="in")



d_TX <- read.csv("TET+TX-100.csv")
head(d_TX)
d_TX$Group <- factor(d_TX$Group, level=c("Ctrl", "0.02", "0.04", "0.2", "1", "5", "10"))

residuals <- (aov(Extracellular ~ Group, data=d_TX))$residuals
shapiro.test(residuals)
bartlett.test(Extracellular ~ Group, data=d_TX)
...
ggsave("TET+TX-100_ex.png", width=690/90, height=420/90, dpi=600, unit="in")
