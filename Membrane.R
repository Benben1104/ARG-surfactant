...
ggsave("TET_Membrane.png", width=565/90, height=360/90, dpi=600, unit="in")



ggplot(d3, aes(Surfactant, Outer.membrane.permeability, fill= Surfactant))+
  theme_bw()+
  scale_y_continuous(limits = c(0, 1.5))+
  geom_jitter(aes(Surfactant, Outer.membrane.permeability), width =0.22, 
              size = 3.75, shape = 21, stroke=0.01, fill="black", alpha=0.95)+
  stat_summary(geom="col", fun="mean",
               position="dodge", width = 0.625, alpha=0.7)+
  stat_summary(geom="errorbar",
               ...
               width=0.15)+
  labs(x=NULL,
       y=expression("Relative change"),
       title=expression("Outer membrane permeability"))+
  scale_fill_manual(values = c("Ctrl"="#B3D9CE", "DTAC"="#203f99", "SDS"="#C31419", "TX-100"="#824CA2"))+
  theme(axis.text.x=element_text(size = 18, face = "bold", vjust = 0))+
  theme(axis.text.y = element_text(size = 18, face = "bold"))+
  theme(axis.title.y = element_text(size=20, face = "bold", vjust = 2))+
...
aov.3 <- aov(Membrane.hydrophobicity ~ Surfactant, data=d3)
summary(aov.3)
TukeyHSD(aov.3)
...
aov.2 <- aov(Membrane.permeability ~ Surfactant, data=d4)
summary(aov.2)
TukeyHSD(aov.2)
...
ggsave("CEF_Membrane.png", width=565/90, height=360/90, dpi=600, unit="in")
